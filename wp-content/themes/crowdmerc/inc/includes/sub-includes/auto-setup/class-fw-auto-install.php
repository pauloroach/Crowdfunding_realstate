...
class FW_Simple_Auto_Install {
		...
		if ( is_array( $result ) ) {
			...
			if ( !empty( $failed_extensions ) ) {
				...
				wp_send_json_error( array(
					'message' => sprintf( esc_html__( 'Failed to activate supported extensions. %s ', 'crowdmerc' ), implode( ', ', $failed_extensions ) ),
				) );
			}
		}
	...
	public function get_setup_messages() {
		return array(
			'plugins_only'		 => sprintf( esc_html__( 'This option will activate Unyson dependencies. The %s demo content will not be installed %s.', 'crowdmerc' ), '<b>', '</b>' ),
			'plugins_and_demo'	 => sprintf( esc_html__( 'This option will activate Unyson dependencies together %s with the demo content %s for the theme.', 'crowdmerc' ), '<b>', '</b>' ),
			'skip_auto_install'	 => esc_html__( 'Skip the auto setup all together and activate all the Unyson dependencies manually. Note that this page will not be  accessible until you install the theme again.', 'crowdmerc' )
		);
	}
	...
	private function admin_enqueue_scripts() {
		...
		$auto_setup_data = array(
			...
			'messages'					 => array(
				'on_leave_alert'	 => esc_html__( 'Attention, the installation process is not yet finished, if leave this page, you lose the information stored on the site!', 'crowdmerc' ),
				'server_problems'	 => esc_html__( "Sorry, we've encountered some errors, try to access this page later.", "crowdmerc" ),
				'process_completed'	 => esc_html__( 'The installation process was completed successfully.', 'crowdmerc' )
			),
	...
	public function auto_setup_page_view() {
		...
		if ( $this->process_is_running() ) {
			...
			echo crowdmerc_return($this->render_view( $this->view_path . '/auto_setup.php' ));
		} else {
			$credentials				 = get_site_transient( $this->credentials_key );
	...
	public function render_install_setup( $params ) {
		...
		echo crowdmerc_return($this->render_view( $this->view_path . '/install_setup.php', $params ));
	}
...
class FW_Admin_Auto_Install extends FW_Simple_Auto_Install {
	...
	public function load_page_tools_callback() {
		...
		if ( !empty( $this->demos ) ) {
			wp_enqueue_script(
			$this->_prefix . '-demo-content-install', $this->get_demo_js_uri(), array( 'jquery', 'underscore', 'crowdmerc' )
			);
			...
			$this->_prefix . '-demo-content-install', 'demo_plugins', array(
				...
				'messages'		 => array(
					'installing'				 => __( 'Installing', 'crowdmerc' ),
					'start_install_plugins'		 => __( 'Installing required plugins ...', 'crowdmerc' ),
					'finish_install_plugins'	 => __( 'Finished installing required plugins', 'crowdmerc' ),
					'start_activate_plugins'	 => __( 'Activaiting required plugins ...', 'crowdmerc' ),
					'finish_activate_plugins'	 => __( 'Finished activaiting required plugins', 'crowdmerc' ),
				)
			)
			);
		}
	...
	public function get_demo_steps() {
		return array(
			'activate-demo-plugins' => array(
				...
				'message'		 => esc_html__( 'Activate required demo plugins', 'crowdmerc' ),
			)
		);
	}
	...
	public function activate_demo_plugins() {
		...
		if ( !empty( $failed_plugins ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to activate required plugins. %s', 'crowdmerc' ), implode( ', ', $failed_plugins ) ),
			) );
		}
	...
	public function get_steps() {
		...
		parent::get_steps(), array(
			'activate-unyson'			 => array(
				...
				'message'		 => esc_html__( 'Activating Unyson', 'crowdmerc' ),
			),
			'activate-required-plugins'	 => array(
				...
				'message'		 => esc_html__( 'Activate required plugins', 'crowdmerc' ),
			)
		)
		);
	}
	...
	public function get_setup_messages() {
		return array(
			'plugins_only'		 => sprintf( esc_html__( 'This option will activate %s already installed plugins and dependencies %s by Network Administrator. The %s demo content will not be installed %s.', 'crowdmerc' ), '<b>', '</b>', '<b>', '</b>' ),
			'plugins_and_demo'	 => sprintf( esc_html__( 'This option will activate Unyson dependencies together %s with the demo content %s for the theme.', 'crowdmerc' ), '<b>', '</b>' ),
			'skip_auto_install'	 => esc_html__( 'Skip the auto setup all together and activate all the plugins manually. Note that this page will not be accessible until you install the theme again.', 'crowdmerc' )
		);
	}
...
class FW_Super_Admin_Auto_Install extends FW_Admin_Auto_Install {
	...
	public function get_demo_steps() {
		return array(
			...
			'install-demo-plugins'	 => array(
				...
				'message'		 => esc_html__( 'Install required demo plugins', 'crowdmerc' ),
			),
		);
	}
	...
	public function install_demo_plugins() {
		...
		if ( !$this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			...
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install required plugins. Folder %s is not writable', 'crowdmerc' ), WP_PLUGIN_DIR ),
			) );
		}
		...
		if ( !empty( $failed_plugins ) ) {
			...
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install required plugins. %s', 'crowdmerc' ), implode( ', ', $failed_plugins ) ),
			) );
		}
		ob_end_clean();
		wp_send_json_success();
	...
	public function get_steps() {
		...
		parent::get_steps(), array(
			'install-unyson'				 => array(
				...
				'message'		 => esc_html__( 'Downloading and installing Unyson', 'crowdmerc' ),
			),
			'install-required-plugins'		 => array(
				...
				'message'		 => esc_html__( 'Installing required plugins', 'crowdmerc' ),
			),
			'install-supported-extensions'	 => array(
				...
				'message'		 => esc_html__( 'Installing supported extensions', 'crowdmerc' ),
			),
			'install-child-theme'			 => array(
				'ajax_action'	 => $this->_prefix . '_install_child_theme',
				'nonce'			 => wp_create_nonce( 'install-child-theme' ),
				'message'		 => esc_html__( 'Downloading and installing child theme', 'crowdmerc' ),
			),
		)
		);
	}
	...
	public function install_supported_extensions() {
		...
		if ( !$this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install supported extensions. Folder %s is not writable', 'crowdmerc' ), WP_PLUGIN_DIR ),
			) );
		}
		...
		if ( is_array( $result ) ) {
			...
			if ( !empty( $failed_extensions ) ) {
				...
				wp_send_json_error( array(
					'message' => sprintf( esc_html__( 'Failed to install supported extensions. %s ', 'crowdmerc' ), implode( ', ', $failed_extensions ) ),
				) );
			}
		}
	...
	public function install_unyson() {
		...
		if ( !$this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install Unyson. Folder %s is not writable', 'crowdmerc' ), WP_PLUGIN_DIR ),
			) );
		}
	...
	public function install_child_theme() {
		...
		if ( !$this->initialize_filesystem( $credentials, wp_normalize_path( get_theme_root() ) ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install Theme. Folder %s is not writable', 'crowdmerc' ), wp_normalize_path( get_theme_root() ) ),
			) );
		}
	...
	public function get_setup_messages() {
		return array(
			'plugins_and_demo'	 => sprintf( esc_html__( 'This option will install and activate all the plugins and dependencies together %s with the demo content %s for the theme.', 'crowdmerc' ), '<b>', '</b>' ),
			'plugins_only'		 => sprintf( esc_html__( 'This option will install and activate all the plugins and dependencies. The %s demo content will not be installed %s.', 'crowdmerc' ), '<b>', '</b>' ),
			'skip_auto_install'	 => esc_html__( 'Skip the auto setup all together and install all the plugins manually. Note that this page will not be accessible until you install the theme again.', 'crowdmerc' )
		);
	}
...
class FW_Installer_Helper {
	...
	public static function activate_plugin( $plugin ) {
		...
		if ( empty( $plugin_data ) ) {
			return array(
				'success'	 => false,
				'data'		 => array(
					'message' => sprintf( esc_html__( 'It was not possible to activate %s. Because it isn\'t installed.', 'crowdmerc' ), $plugin[ 'name' ] )
				)
			);
		}
	...
	public static function activate_plugins( $plugins ) {
		...
		if ( !current_user_can( 'activate_plugins' ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Current user can\'t activate plugins.', 'crowdmerc' ), WP_PLUGIN_DIR ),
			) );
		}
	...
	private static function plugins_api( $action, $args = null ) {
		...
		if ( false === $res ) {
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="http://wordpress.org/support/">support forums</a>.', 'crowdmerc' ), $request->get_error_message() );
			} else {
				$res = maybe_unserialize( wp_remote_retrieve_body( $request ) );
				if ( !is_object( $res ) && !is_array( $res ) ) {
					$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="http://wordpress.org/support/">support forums</a>.', 'crowdmerc' ), wp_remote_retrieve_body( $request ) );
				}
			}
			...
			if ( !empty( $plugin[ 'version' ] ) ) {
				if ( version_compare( $installed[ $plugin_file[ 0 ] ][ 'Version' ], $plugin[ 'version' ], '<' ) ) {
					...
					return array(
						'success'	 => true,
						'data'		 => array(
							'message' => esc_html__( 'Plugin is already installed.', 'crowdmerc' )
						)
					);
				}
			} else {

 ...
 */
class Crowdmerc_recent_post extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'xs-recent-post', esc_html__( 'Crowdmerc Recent Post', 'crowdmerc' ), $widget_opt );
	}
		...
		<ul class="unstyled clearfix xs-recent-post-widget">
					...
					<li>
						...
						<div class="post-info float-right w-75">
							<div class="post-meta xs-font-alt text-uppercase xs-mb-10">
								<span class="post-author"><?php echo esc_html__( 'Posted by','crowdmerc' ) ?> <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) ?>"><?php echo get_the_author(); ?></a></span>
			...
			<?php else: ?>
				<div class="nopost_message">
					<p><?php echo esc_html__( 'No Post Available', 'crowdmerc' ) ?></p>';
				</div>
			<?php endif; ?>  
		</ul>
		<?php
	...
	function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = esc_html__( 'Recent Posts', 'crowdmerc' );
		}
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number_of_posts' ) ); ?>"><?php esc_html_e( 'Number Of Posts:', 'crowdmerc' ); ?></label>
